   ...
   stages {
      ...
      stage('Deployment - Kubernetes Cluster Resource Creation/Update') {
         steps {
            ...
            echo 'kubectl set image deployment/deloitte-drifters-frontend deloitte-drifters-frontend=gcr.io/dtc-user21/jenkins-pipe-external:v1.${env.BUILD_ID} --record --namespace=dev'
            sh "kubectl set image deployment/deloitte-drifters-frontend deloitte-drifters-frontend=gcr.io/dtc-user21/jenkins-pipe-external:v1.${env.BUILD_ID} --record --namespace=dev"
